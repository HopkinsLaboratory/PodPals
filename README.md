...
4. **Running the Analysis:**
    ...
    - The comparison process then continues by calculating similarities between the first identified unique geometry (the first unique $\vec{V}_b$ identified when comparising against $\vec{V}_a$) and all other unique geometries, excluding the initial file ($\vec{V}_a$). This iterative approach repeats, exluding all duplicates and the initial vector $\vec{V}_a$ until no additional pairwise comparisons can be performed.
