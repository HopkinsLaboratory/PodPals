...
def ORCA_opt_plt(file, save_plot = False):
    ...
    if not file.endswith('.out'):
        print(f'{datetime.now().strftime("[ %H:%M:%S ]")} Analysis the the optimization can only be performed on ORCA files with a .out extension.')
        return
    ...
    try:
        ...
        raise ValueError(f'{datetime.now().strftime("[ %H:%M:%S ]")} Problem extracting energies from {file}: {e}')

    #if no SCF cycles have been completed, the job probably didn't start correctly. Throw a value error to the user
    if len(energy) == 0:
        raise ValueError(f'{datetime.now().strftime("[ %H:%M:%S ]")} No SCF cycles have been completed in your calculation! Did it start correctly?')
